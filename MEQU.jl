...
function create_buyers(num_buyers::Int64)::Vector{buyer}
    ...
    for b in 1:num_buyers
        new_buyer = buyer(b, b, 1.0, b * 1.0, false, 0.0, 0.0)
...
function update_quality_expectation(buyers::Vector{buyer}, network; accessibility_step::Int64=1, λ_ind::Float64=0.0, λ_neigh::Float64=0.0)
    ...
    for i in 1:length(buyers)
        ...
        if getfield(buyers[i], :unit_bought)
        ...
        new_quality_expectation = buyers[i].quality_expectation + λ_ind * my_prod_qual + λ_neigh * mean_neigh_prod_qual
...
function simulate(max_iter::Int64, num_sellers::Int64, num_buyers::Int64; network_type::String, num_links::Int64=0, pref_attachment_links::Int64=0, dist::String, quality_variance::Float64, accessibility_step::Int64=1, λ_ind::Float64, λ_neigh::Float64)
    """SETUP"""
    ...
    for iteration in 1:max_iter
        ...
        buyers, sellers = exchange_goods(buyers, sellers, market; dist = dist, quality_variance = quality_variance)

        push!(result_buyers_surplus, sum(getfield.(buyers, :surplus)))
        push!(result_sellers_surplus, sum(getfield.(sellers, :surplus)))

        buyers = update_quality_expectation(buyers, network; accessibility_step = accessibility_step, λ_ind = λ_ind, λ_neigh = λ_neigh)
...
QUALITY_VARIANCE = 0.2 # variance of quality
ACCESSIBILITY_STEP = 1 # distance from agent to agent, defining neighbourhood
Λ_IND = 0.1 # individual learning
Λ_NEIGH = 0.1 # social learning

# SIMULATION

RESULTS = simulate(MAX_ITER, NUM_SELLERS, NUM_BUYERS; network_type=NETWORK_TYPE, pref_attachment_links = PREF_ATTACHMENT_LINKS, dist = DIST, quality_variance =  QUALITY_VARIANCE, accessibility_step = ACCESSIBILITY_STEP, λ_ind = Λ_IND, λ_neigh = Λ_NEIGH)
